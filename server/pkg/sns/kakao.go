...
func VerifyKakaoToken(ctx context.Context, accessToken string) (*KakaoUserInfo, error) {
	...
	go func() {
		...
		if err != nil {
			resultCh <- apiResult{nil, fmt.Errorf("failed to send request: %w", err)}
			return
		}
		...
		if resp.StatusCode != http.StatusOK {
			...
			resultCh <- apiResult{nil, fmt.Errorf("kakao API returned status %d: %s", resp.StatusCode, string(body))}
			return
		}

		var kakaoResp KakaoAPIResponse
		if err := json.NewDecoder(resp.Body).Decode(&kakaoResp); err != nil {
			resultCh <- apiResult{nil, fmt.Errorf("failed to decode response: %w", err)}
			return
		}

		resultCh <- apiResult{&kakaoResp, nil}
	}()
