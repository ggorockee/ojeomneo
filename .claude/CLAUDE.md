...
| 모드 | 대상 | 메시지 스타일 |
|------|------|---------------|
| **Debug (kDebugMode)** | 개발자 | 상세 기술 정보 포함 (`[DEV] HTTP 오류: 500`) |
| **Release** | 사용자 | Naver 스타일 친화적 메시지 (`일시적인 오류가 발생했어요.`) |

- 모든 에러 메시지는 **한글**로 표시
- `lib/utils/app_messages.dart`에서 중앙 관리
- 사용자 메시지는 친근하고 공감하는 톤 유지
