...
> **설계 변경**: 기존 3개 추천에서 **1개 추천**으로 단순화. 선택의 역설 방지 및 UX 집중.
...
> **변경사항**: `alternatives` 제거, 단일 추천(`recommendation`)으로 단순화
...
| 유형 | 설명 | 저장 위치 |
|------|------|----------|
| 스케치 이미지 | 사용자가 그린 그림 | Cloudflare R2 |
| 메뉴 이미지 | 음식 사진 (관리자 업로드) | Cloudflare R2 |

### 9.2 Cloudflare R2 Direct Upload 방식

**선택 이유**:

| 항목 | 설명 |
|------|------|
| 비용 | S3 대비 egress 비용 무료, 저렴한 storage |
| 성능 | Cloudflare CDN 자동 통합, 전 세계 엣지 캐싱 |
| 보안 | Pre-signed URL로 직접 업로드, 서버 부하 감소 |
| 호환성 | S3 호환 API, 기존 SDK 활용 가능 |

**업로드 플로우**:

```
[Mobile] → POST /upload/presign → [Server]
                                      │
                                      ▼
                              Cloudflare R2 API
                              (Pre-signed URL 생성)
                                      │
                                      ▼
[Mobile] ← { upload_url, file_key } ←─┘
    │
    ▼
PUT (upload_url) with image data → [Cloudflare R2]
    │
    ▼
[Mobile] → POST /sketch/analyze { file_key } → [Server]
```
...
| 항목 | 값 |
|------|-----|
| Endpoint | `POST /ojeomneo/v1/upload/presign` |
...
| 필드 | 타입 | 필수 | 설명 |
|------|------|------|------|
| type | String | O | sketch / menu |
| content_type | String | O | image/png, image/jpeg |
...
```json
{
  ...
  "data": {
    ...
    "file_key": "sketches/2024/01/uuid.png",
...
| 방식 | 장점 | 단점 | 적합성 |
|------|------|------|--------|
| **Cloudflare R2 (선택)** | egress 무료, CDN 통합 | R2 API 학습 필요 | ⭐ 최적 |
| AWS S3 | 안정성, 레퍼런스 풍부 | egress 비용 높음 | 대규모 트래픽 시 비용 부담 |
| Firebase Storage | Flutter 통합 쉬움 | 비용 예측 어려움 | 소규모 프로젝트 적합 |
| 서버 직접 저장 | 단순 구현 | 서버 부하, 확장성 한계 | 비권장 |
...
관리자 페이지에서 iOS/Android 앱의 최소 버전을 관리하고, 강제 업데이트를 제어하는 시스템.
...
| min_version | VARCHAR(20) | 최소 허용 버전 (예: 1.2.0) |
| latest_version | VARCHAR(20) | 최신 버전 |
| force_update | BOOLEAN | 강제 업데이트 활성화 여부 |
| store_url | TEXT | 앱스토어/플레이스토어 URL |
| update_message | TEXT | 업데이트 안내 메시지 |
| is_active | BOOLEAN | 활성화 여부 |
| updated_at | TIMESTAMP | 수정일시 |
| updated_by | BIGINT | 수정자 FK → admin_users |
...
| 항목 | 값 |
|------|-----|
| Endpoint | `GET /ojeomneo/v1/app/version` |
| Auth | 불필요 (앱 시작 시 호출) |
...
| current_version | String | O | 현재 앱 버전 (예: 1.0.0) |
...
```json
{
  ...
  "data": {
    ...
    "update_message": "더 나은 서비스를 위해 최신 버전으로 업데이트해 주세요."
  }
}
```

### 10.4 Mobile 강제 업데이트 UI

**트리거 조건**: `current_version < min_version && force_update == true`
...
| 팝업 타입 | 중앙 모달 (닫기 불가) |
| 배경색 | 흰색 (#FFFFFF) |
| 오버레이 | 회색 반투명 (rgba(0,0,0,0.5)) |
| 버튼 | "업데이트 하러가기" (Primary) |
| 버튼 액션 | store_url로 이동 |

**플로우**:

```
[앱 시작] → [Splash] → GET /app/version
                            │
                ┌───────────┴───────────┐
                │                       │
        force_update=false      force_update=true
        or version OK           and version < min
                │                       │
                ▼                       ▼
           [홈 화면]            [강제 업데이트 팝업]
                                        │
...
| 최소 버전 | Text | Semantic versioning (X.Y.Z) |
| 최신 버전 | Text | 스토어 배포 버전 |
| 강제 업데이트 | Toggle | ON/OFF |
| 스토어 URL | URL | 앱스토어/플레이스토어 링크 |
| 안내 메시지 | Textarea | 사용자에게 표시할 메시지 |
